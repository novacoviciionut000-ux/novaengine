...
//VERY IMPORTANT FUNCTION BELOW !!!!! This basically handles the entire game interactivity, it handles the input, it handles the camera movement, it handles the entity updates, it handles the world to camera space transformation, it basically does everything that is needed for the game to run, so this function is very important and should be well understood by anyone who wants to understand how the game works
