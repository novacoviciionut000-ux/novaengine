...
void gameLoop(){
    ...
    //end starfield
    //entity creation
        entity_t *gun = init_gun("models/gun.obj");
        printf("gun initialized\n");
        if(!gun){
            printf("Couldn't find model for gun\n");
            goto CLEANUP;
        }
        if(!add_object(scene, (void*)gun, ENTITY_TYPE))goto CLEANUP;
      entity_t *bunny2 = (entity_t*)get_obj("models/Rtwo_low.obj", (vec4_t){{{1.0f,0.2f,0,0}}} , neon_green, 0.003f, true,0,0,0, false, ENTITY_TYPE);
      ...
      //  entity_t *landscape = (entity_t*)get_obj("models/landscape.obj", (vec4_t){{{0,-5.0f,0,0}}} , light_brown, 1.0f, false,0,M_PI,M_PI, false, ENTITY_TYPE);
