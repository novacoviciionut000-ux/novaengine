...
scene_t *create_scene(void) {
    ...
    if (!scene->entity_manager   || !scene->particle_manager ||
        ...
        printf("ERROR: Failed to allocate one or more base managers!\n");
        goto CLEANUP;
    }
    ...
    if (!scene->entity_manager->entities) {
        printf("ERROR: Failed to allocate entity_manager->entities!\n");
        goto CLEANUP;
    }
    ...
    if (!get_particles(scene->particle_manager)) {
        printf("ERROR: get_particles() failed!\n");
        goto CLEANUP;
    }
    ...
    if (!scene->camera_manager->camera || !scene->camera_manager->meshes) {
        printf("ERROR: Failed to create camera or allocate meshes!\n");
        goto CLEANUP;
    }
    ...
    if (!scene->render_manager->triangles    || !scene->render_manager->render_usage  ||
        ...
        printf("ERROR: Failed to allocate render_manager buffers!\n");
        goto CLEANUP;
    }
    ...
    if(!scene->enemy_manager->enemies || !scene->enemy_manager->free_enemies) {
        printf("ERROR: Failed to allocate enemy arrays!\n");
        goto CLEANUP;
    }
    scene->enemy_manager->num_enemies       = 0;
    scene->enemy_manager->free_enemies_count = 0;      
    scene->enemy_manager->allocated_enemies  = MAX_ENEMIES;
    
    initialize_enemies(scene->enemy_manager);
    
    if (!scene->enemy_manager->enemies || !scene->enemy_manager->free_enemies) {
        printf("ERROR: Enemy arrays became NULL after initialize_enemies!\n");
        goto CLEANUP;
    }
    ...
    if(!scene->collision_manager->collision_boxes) {
        printf("ERROR: Failed to allocate collision_boxes!\n");
        goto CLEANUP;
    }
    ...
    printf("SUCCESS: create_scene completed successfully.\n");
    return scene;

CLEANUP:
    printf("CLEANUP: Destroying partially initialized scene.\n");
    destroy_scene(scene);
    return NULL;
}
...
void destroy_scene(scene_t *scene) {
    ...
    if (scene->enemy_manager) {
        /* free_enemies[] holds ALL enemy allocations */
